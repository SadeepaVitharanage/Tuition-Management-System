...
export const generateDateRangePDF = (report, from, to) => {
  ...
  let y = 40;
  y = addSummaryCards(doc, [
    { label: 'Total Payments', value: report.totalPayments?.toString() || '0' },
  ...
  const afterFirst = doc.lastAutoTable.finalY + 8;
  doc.setTextColor(...BRAND_COLOR);
  doc.setFontSize(10);
  doc.setFont('helvetica', 'bold');
  doc.text('Payment Details', 14, afterFirst + 4);
  ...
  autoTable(doc, {
    ...
    headStyles: { fillColor: BRAND_COLOR, textColor: [255, 255, 255], fontSize: 7, fontStyle: 'bold' },
...
export const generateTeacherPDF = (report) => {
  const doc = new jsPDF();
  addHeader(doc, 'Teacher Income Report', `${report.teacher?.name} • ${report.month}`);
  ...
  y = addSummaryCards(doc, [
    { label: 'Classes', value: report.totalClasses?.toString() || '0' },
