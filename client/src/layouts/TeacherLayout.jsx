...
import { LayoutDashboard, ClipboardCheck, LogOut, Menu, School } from 'lucide-react';
        ...
        <div className="px-2 py-3 border-t border-white/10">
          {!collapsed && (
            <div className="px-3 py-2 mb-2">
              ...
              <span className="mt-1 inline-block bg-[#F5C518] text-[#0d6b7a] text-xs font-bold px-2 py-0.5 rounded-full">TEACHER</span>
            </div>
          )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white h-16 px-6 flex items-center justify-between border-b border-gray-200 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-gray-800">{navItems.find(n => n.path === location.pathname)?.label || 'Teacher Portal'}</h1>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-sm font-semibold text-gray-800 leading-none">{user?.name}</p><p className="text-xs text-gray-400 mt-0.5">Teacher</p></div>
          </div>
        </header>
